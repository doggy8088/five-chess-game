...
- 困難模式的 AI 同時具備必殺威脅攻防與威脅感知搜索（alpha-beta，深度 3），會避開黑棋禁手、主動防禦對手的活叉與雙威脅（含跳三、跳四等斷點棋型）。
...
| 中等 | 貪婪評估＋立即取殺／擋殺。 |
| 困難 | 必殺威脅攻防（活四／跳四／雙四）＋威脅感知 alpha-beta 搜索（深度 3、節點預算上限）。棋型評估採整線樣式比對，能辨識跳三 `●●_●`、跳四 `●●_●●` 等帶斷點的威脅，並依連珠規則避開黑棋禁手。 |
